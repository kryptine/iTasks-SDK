...
Start :: *World -> *World
Start world 
	= startEngine 	[	publish "/" (\_ -> loginAndManageWorkList title basicAPIExamples <<@ ApplyLayout (setUIAttributesRule (titleAttr title)))
...
externalProcessExample =
    ...
    withShared
        Nothing
        ( \sds -> ( externalProcess () path [] Nothing sds handlers Nothing gEditor{|*|} <<@ ApplyLayout (removeSubUIsRule (SelectByPath [])) >&>
