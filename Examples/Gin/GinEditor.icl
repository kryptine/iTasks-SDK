...
getConfig = undef // TODO accWorld ginLoadConfig >>= \mbConfig -> case mbConfig of
...
////where
	...
	////noActions` = undef // TODO noActions
...
//activator taskList = undef // forever activator` 
////where
	////stateShared = taskListState taskList
	
	////activator` :: Task ParallelControl
	////activator` =	(viewSharedInformation "Diagram monitor" [] stateShared Void >? \(state,_) -> state.dirty) //Look for the dirty flag to become True
...
////actions :: (TaskList EditorState) -> [(Action, Task ParallelControl)] TODO
//actions taskList
	//=	[ (ActionNew,              undef) // TODO actionTask (\s -> askSaveIfChanged s >>| getInitialState))
		//, (ActionOpen,             actionTask open)
		//, (ActionSave,             actionTask save)
		//, (ActionSaveAs,           actionTask saveAs)
		//, (ActionCompile,          actionTask compile)
		//, (ActionQuit,             undef) // TODO return Stop)
	...
	//where
		...
		//addTask task
			//=	undef // TODO defaultInteractionLayout
		...
		//moduleEditor title v = undef // TODO addTask (updateSharedInformation title [UpdateView (app2 (GetShared,\f -> PutbackShared (\a _ e -> f a e)) (liftModuleView v))] stateShared Void)
		...
		//importsEditor = addTask 
			...
			//)
		//sourceView = undef // TODO addTask (viewSharedInformation "source view" [ShowView (GetShared (\s -> formatSource s.EditorState.source))] stateShared Void)
...
////TODOgVisualizeHtml{|TypeExpressionView|} _ (TypeExpressionView v) = [RawText v]
...
////TODOderive gDefaultMask TypeExpressionView
...
//askSaveIfChanged state = undef // TODO if state.changed
...
//where
	...
	//requestConfirmation message = undef // TODO viewInformation message Void >>+ \_ -> UserActions [(ActionYes, True), (ActionNo, False)]
...
//= accIWorld (batchBuild state.EditorState.gMod)
  //>>= \result = case result of
	//CompileSuccess dynfile 	->	viewInformation ("Compiler output", "Compiled successfully. Click \"Refresh workflows\" to view the task") [] Void 
								...
								//>>= \task -> undef // TODO addWorkflow (makeWorkflow state task)
                                     //>>| return { state & compiled = Just dynfile }
    //error					->  undef // TODO viewInformation "Compiler output" [About error] Void >>| return state
//where
...
//where
	//eval taskNr iworld
		...
		//= undef // TODO (TaskFinished res,iworld)
