...
googleMapsTasklet (cla,clo) zoom = 
	...
	, resultFunc		= \{waypoints,plane,initialized} = if initialized (Value (waypoints,plane) False) NoValue
...
where
	...
	where
		updatePerspective st=:{GoogleMapsState|map = Just map} _ _  d 
			...
			= (d, {GoogleMapsState| st & center=(fromHtmlObject la, fromHtmlObject lo)})	
