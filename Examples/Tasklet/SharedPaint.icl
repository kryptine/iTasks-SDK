...
painterEditlet :: Editlet Drawing [DrawType]
painterEditlet = Editlet (Drawing []) serverDef clientDef
where
	serverDef =
	  {  EditletServerDef
	  |  genUI   = painterGenerateGUI
	  ,  defVal  = Drawing []
	  ,  genDiff = \_ (Drawing ds) -> Just ds
	  ,  appDiff = \ds _ -> Drawing ds
	  }
	...
	clientDef =
	  ...
	  ,  genDiff  = \_ cl -> Just cl.draw
	  ,  appDiff  = \ds cl -> {cl & draw = ds}
	  }

	updateUI cid mbDiff cl jsworld = (cl, jsworld)
...
painterGenerateGUI _ world  
	...
	# eventHandlers = [
			ComponentEvent "editlet" "init" onStart,
...
taskletExamples =
	[workflow "Painter tasklet" "Simple painter tasklet" editlet]

editlet = updateInformation "CodeMirror" [] painterEditlet
