...
//  variant 3: the board, player name, player glyph are separate tasks, one locally known shared state:
...
play_game who shared_game
	...
	>>= \game -> let player = if (who === Tic) game.players.tic game.players.tac in
	             player @: (
	                  getTags 3 >>= \[t1,t2,t3] -> 
                      ( tagTask t1 (updateSharedInformation () [imageUpdate toViewModel (\game _ -> render who game) handleConflict fromViewModel] shared_game)
	                       -||
	                    tagTask t2 (viewSharedInformation () [ViewWith (\game -> name_of game.turn game.players)] shared_game)
	                       -||
	                    tagTask t3 (viewSharedInformation () [imageView (\game _ -> glyph game.turn) handleConflict] shared_game)
