implementation module GUIWidgets
...
gForm{|HtmlCheckbox|} (init,formid =: {FormId|mode}) hst =:{HSt|cntr,prefix}
...
gForm{|HtmlRadiogroup|} (init,formid =: {FormId|mode}) hst =: {cntr,prefix} 	
	...
	= ({ changed		= False
	   ...
	   , inputs			= [{formid = formid.FormId.id, inputid = inputid, prefix = prefix, type = "HtmlRadiogroup", updateon = (if (mode == Submit) OnSubmit OnChange), value = toString cur, options = Nothing}]
	   },setHStCntr (cntr + 1) hst)
where
	(HtmlRadiogroup options cur)	= formid.ival

gForm{|HtmlTextarea|} (init,formid =: {FormId|mode=Display}) hst =: {cntr,prefix} 	
	= ({ changed		= False
	   , value			= formid.ival
...
gForm{|HtmlTextarea|} (init,formid =:{FormId|mode}) hst =:{cntr,prefix}
	# inputid = mkInputId formid cntr
	= (	{ changed			= False
		, value				= formid.ival
		, form				= [SpanTag [IdAttr (prefix +++ inputid)] []]
		, inputs			= [{formid = formid.FormId.id, inputid = inputid, prefix = prefix, type = "HtmlTextarea", updateon = (if (mode == Submit) OnSubmit OnChange), value = val, options = Nothing}]
		},setHStCntr (cntr + 1) hst)
where
	(HtmlTextarea rows val) = formid.ival


gForm{|HtmlPassword|} (init,formid =: {FormId|mode}) hst =: {cntr,prefix} 	
	#inputid = mkInputId formid cntr
	= ({ changed		= False
	   , value			= formid.ival
	   , form			= [SpanTag	[IdAttr (prefix +++ inputid)] []]
	   , inputs			= [{formid = formid.FormId.id, inputid = inputid, prefix = prefix, type = "HtmlPassword", updateon = (if (mode == Submit) OnSubmit OnChange), value = v, options = Nothing}]
	   },setHStCntr (cntr + 1) hst)
where
	(HtmlPassword v)	= formid.ival

gForm {|HtmlTime|} (init,formid =:{FormId|mode}) hst
...
gForm {|HtmlDate|} (init,formid =:{FormId|mode}) hst
...
gForm {|HtmlCurrency|} (init,formid =:{FormId|mode}) hst
	= case mode of
		Display
			= ({ changed	= False
			   , value		= formid.ival
...
gForm {|HtmlTimer|} (init, formid) hst =: {cntr,prefix}
	...
	= ({ changed		= False
	   , value			= formid.ival
	   , form			= []
	   , inputs			= [{formid = formid.FormId.id, inputid = inputid, prefix = prefix, type = "HtmlTimer", updateon = OnTimeout, value = toString val, options = Nothing}]
	   }, hst)
where
