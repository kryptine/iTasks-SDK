...
where
	eval taskId iworld
		...
		= case val of
			Ok (ReadResult val)		= (Ok (fun (readSymbols val)), iworld) 
...
where
	eval event evalOpts state iworld
                # (val, iworld) = read symbolsShare EmptyContext iworld
                = case val of
                        Ok (ReadResult val)          = let (Task eval`) = taskfun (fst (copy_from_string (base64Decode val))) in eval` event evalOpts state iworld
