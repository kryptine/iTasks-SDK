...
where
	eval taskId iworld
		...
		= case val of
			Ok (Result val)		= (Ok (fun (fst (copy_from_string (base64Decode val)))), iworld) 
...
where
	eval event evalOpts state iworld
                # (val, iworld) = read symbolsShare EmptyContext iworld
                = case val of
                        Ok (Result val)          = let (Task eval`) = taskfun (fst (copy_from_string (base64Decode val))) in eval` event evalOpts state iworld
