...
tFApp inh eid fn args assoc tsrc
  ...
  | otherwise
      ...
      #! box = tRoundedRect (textxspan ArialRegular10px pp + px 10) (px (getfontysize ArialRegular10px + toMilliInt 10)) <@< { dash = [5, 5] }
...
tArrowTip color
  = polygon [ (px 0, px 0), (px 8, px 4), (px 0, px 8) ] <@< { stroke = TonicBlack }
...
tHorizConn Nothing  = xline (px 8)
tHorizConn (Just c) = rect (px 8) (px 3) <@< { fill = c }

tShortHorizConn :: !(Maybe SVGColor) -> Image ModelTy
tShortHorizConn Nothing  = xline (px 4)
tShortHorizConn (Just c) = rect (px 4) (px 3) <@< { fill = c }
...
tCaseDiamond inh exprImg [(diamondTag, uDiamondTag) : tsrc]
  ...
  #! imgWidth     = maxSpan [px 50, imagexspan diamondTag]
  #! edgeMargin   = imgHeight *. 2.0
  #! centerX      = (imgWidth /. 2.0) + edgeMargin
  #! leftCorner   = (px 0, px 0)
  #! topCorner    = (centerX, ~ edgeMargin)
  #! rightCorner  = (centerX *. 2, px 0)
...
tLet inh pats expr [(txttag, uTxtTag) : tsrc]
  ...
  = case expr of
      ...
      _
        ...
        #! letWidth        = imagexspan txttag + px 8
        #! letHeight       = imageyspan txttag + px 8
        ...
        #! linePart        = case t.syn_status of
                               TNotActive -> xline ((letWidth - px 8) /. 2)
                               _          -> rect  ((letWidth - px 8) /. 2) (px 3) <@< { fill = case inh.inh_prev_statstab of
        ...
        #! lineAct              = case t.syn_status of
                                    ...
                                    _          -> case inh.inh_prev_statstab of
                                                    ...
                                                    (_, TUnstable) -> Just TonicGreen
                                                    _              -> Just TonicWhite
        #! connBox         = beside (repeat AtMiddleY) [] Nothing [] [linePart, rect (px 8) (px 8), linePart] NoHost
        #! letImg          = above  (repeat AtMiddleX) [] Nothing [] [letImg, yline (px 8), connBox, empty zero (letHeight + px 8)] NoHost
...
renderParallelContainer inh eid moduleName taskName descr syn_branches uContextTag tsrc
  ...
  #! valNodeIsSelected = case inh.inh_selDetail of
                           ...
                           _ = False
  #! valAnchor         = rect (px 8) (px 8) <@< { onNclick = openDetails clickMeta, local = False }
                                            ...
                                            <@< { strokewidth = if valNodeIsSelected (px 3) (px 1) }
  ...
  tParApp isCompact eid parentModName parentFuncName taskName syn_branches [(tntag, uTnTag) : (argstag, uArgsTag) : tsrc]
    #! taskNameImg = tag uTnTag (margin (px 2.5, px 5) (text ArialBold10px taskName))
...
tTaskDef inh moduleName taskName resultTy args argvars tdbody [(nameTag, uNameTag) : (argsTag, uArgsTag) : (bdytag, uBodyTag) : tsrc]
  #! userImg      = case inh.inh_bpinst of
                      Just bpi -> case getCurrentUser bpi of
                                    Just cu -> beside (repeat AtMiddleY) [] Nothing [] [margin (px 0, px 0, px 0, px 8) littleman, text ArialRegular10px (" " +++ cu)] NoHost
                                    _       -> empty zero zero
                      _        -> empty zero zero
  ...
  #! taskNameImg  = case inh.inh_bpinst of
                      Just bpi -> case getCurrentUser bpi of
                                    Just _ -> tag uNameTag (margin (px 2, px 5) taskNameImg)
                                    _      -> tag uNameTag (margin (px 5) taskNameImg)
                      _ -> tag uNameTag (margin (px 5) taskNameImg)
  #! binds        = flatten (strictTRZipWith3 mkArgAndTy args [0..] (strictTRMap Just argvars ++ repeat Nothing))
  #! argsText     = 'GS'.grid (Columns 4) (RowMajor, LeftToRight, TopToBottom) [] [] [] [] (strictTRMap (margin (px 1, px 0)) binds) NoHost
  #! argsImg      = tag uArgsTag (margin (px 5) argsText)
  #! taskBodyImgs = tag uBodyTag (margin (px 5) tdbody)
...
tDefaultMApp` isDynamic inBranch isCompact isActive wasActive isInAccessible isUnreachable eid parentModName parentFuncName modName taskName taskArgs augments [(tntag, uTnTag) : (argstag, uArgsTag) : tsrc]
  #! taskNameImg       = tag uTnTag (margin (px 2.5, px 5) (beside (repeat AtMiddleY) [] Nothing [] [text ArialBold10px taskName : text ArialRegular10px " " : intersperse (text ArialRegular10px " ") augments] NoHost))
  ...
  #! strokeColor       = if isDynamic
                           ...
                           TonicBlack
  #! strokeWidth       = if (isDynamic && (futureUnreachable || futureReachable) && inBranch) (px 3) (px 1)
  = case taskArgs of
      []
      ...
      taskArgs
        #! argsImg   = tag uArgsTag (margin (px 2.5, px 5) (above (repeat AtLeft) [] Nothing [] (strictTRMap (margin (px 1, px 0)) taskArgs) NoHost))
        #! maxXSpan  = maxSpan [imagexspan tntag, imagexspan argstag]
        #! txtBgRect = tRoundedRect maxXSpan (px 15) <@< { fill = bgColor }
                                                     <@< { stroke = strokeColor }
                                                     <@< { strokewidth = strokeWidth }
...
tAssign inh lhsExpr assignedTask [(assignTaskTag, uAssignTaskTag) : (headerTag, uHeaderTag) : tsrc]
  ...
  #! assignedTaskImg      = tag uAssignTaskTag (margin (px 5) assignedTask.syn_img)
  #! maxXSpan             = maxSpan [imagexspan headerTag, imagexspan assignTaskTag]
  #! taskNameImg          = margin (px 5) (text ArialBold10px (user +++ if (desc == "") "" (": " +++ desc)))
...
addAction (Just (action, enabled)) arr (t, uT)
  #! l = tag uT (margin (px 3) (beside (repeat AtMiddleY) [] Nothing [] [littleman, tuneIf (not enabled) (text ArialBold10px (" " +++ action)) {fill = toSVGColor "#666"}] NoHost))
  #! l` = overlay (repeat (AtMiddleX, AtMiddleY))  [] [ rect (imagexspan t + px 5) (imageyspan t + px 5) <@< {fill        = toSVGColor (if enabled "#ebebeb" "#fff")}
                                                                                                         <@< {strokewidth = px 1}
                                                                                                         <@< {stroke      = toSVGColor (if enabled "#000" "#ccc")}
                                                                                                         <@< {dash        = if enabled [] [5, 5] }
                                                                , l] NoHost
  = beside (repeat AtMiddleY) [] Nothing [] [l`, arr] NoHost
addAction _ _ _ = empty (px 0) (px 0)
...
tBranches inh mkBranch needAllDone inclVertConns exprs contextTag tsrc
  ...
  iter existsSomeActivity maxXSpan ((pat, texpr, showRhs, unreachable), currBranchActivity, (imgTag, uImgTag)) tsrc
    ...
    #! img          = case showRhs of
                        True
                          #! lhs       = tag uImgTag (margin (px 2.5, px 0) lhs)
                          #! lineWidth = (maxXSpan - imagexspan imgTag) + px 8
                          #! rhs       = case syn.syn_status of
                                           TAllDone ->
                                             case syn.syn_stability of
                                               TNoVal    -> rect lineWidth (px 3) <@< { fill = TonicWhite }
                                               TStable   -> rect lineWidth (px 3) <@< { fill = TonicBlue }
                                               TUnstable -> rect lineWidth (px 3) <@< { fill = TonicGreen }
                                               _         -> xline lineWidth
  ...
  mkVertConn ts
    | length ts < 2 = empty (px 0) (px 0)
    | otherwise
        #! firstTag   = hd ts
        #! lastTag    = last ts
        #! allYSpans  = foldl (\acc x -> imageyspan x + acc) (px 0) ts
...
tTextWithGreyBackground font txt
  #! textWidth = textxspan font txt + px 10
  = overlay (repeat (AtMiddleX, AtMiddleY)) [] [rect textWidth (px (getfontysize font + toMilliInt 10)) <@< {fill = toSVGColor "#ebebeb"} <@< {strokewidth = px 0}, text font txt] NoHost

littleman :: Image a
littleman
  #! head      = circle (px 8)       <@< {StrokeWidthAttr | strokewidth = px 0}
  #! shoulders = rect (px 16) (px 4) <@< {StrokeWidthAttr | strokewidth = px 0}
                                     <@< { xradius     = px 5 }
                                     <@< { yradius     = px 5 }
  #! arm       = rect (px 3) (px 6)  <@< {StrokeWidthAttr | strokewidth = px 0}
  #! vline     = yline (px 4)        <@< {StrokeAttr | stroke = TonicWhite}
                                     <@< {StrokeWidthAttr | strokewidth = px 2}
  #! chest     = rect (px 10) (px 5) <@< {StrokeWidthAttr | strokewidth = px 0}
  #! arms      = beside [] [] Nothing [(px 0, px -2), (px 0, px 0), (px 0, px -1), (px 0, px 0), (px 0, px -2)] [arm, vline, chest, vline, arm] NoHost
...
tExceptionBox = overlay (repeat (AtMiddleX, AtMiddleY)) [] [ rect (px 8) (px 8) <@< { fill = TonicRed }
...
tStableBox = overlay (repeat (AtMiddleX, AtMiddleY)) [] [ rect (px 8) (px 8) <@< { fill = TonicBlue }
...
tUnstableBox = overlay (repeat (AtMiddleX, AtMiddleY)) [] [ rect (px 8) (px 8) <@< { fill = TonicGreen }
