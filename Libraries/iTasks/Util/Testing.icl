...
where
	runTest options (results,(console,world)) {UnitTest|name,test}
		...
		| otherwise
			# console = fwrites (toString (toJSON (StartEvent {StartEvent|name=name,module_name=Nothing})) +++ "\n") console
			...
			# message = case result of
				...
				Failed _ = "FAILED"
				Skipped = "SKIPPED"
			# console = fwrites (toString (toJSON (EndEvent {EndEvent|name=name,module_name=Nothing,event=result,message=message})) +++ "\n") console
