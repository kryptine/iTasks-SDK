...
externalProcess poll cmd args dir mopts sdsin sdsout = Task (wrapOldStyleTask eval)
where
...
tcplisten port removeClosed sds handlers = Task (wrapOldStyleTask eval)
where
...
tcpconnect host port sds handlers = Task (wrapOldStyleTask eval)
where
    eval DestroyEvent evalOpts tree=:(TCBasic taskId ts _ _) iworld=:{ioStates}
        # ioStates = case 'DM'.get taskId ioStates of
            Just (IOActive values)  = 'DM'.put taskId (IODestroyed values) ioStates
