...
where
	...
	= case 'SDS'.read shared ('SDS'.TaskContext taskId) iworld of
		...
		(Ok (ReadResult val), iworld)
			= trace_n ("Read from " +++ sdsIdentity shared) (ValueResult (Value val True) evalInfo (ReplaceUI (ui UIEmpty)) (TCStable taskId ts (DeferredJSON val)), iworld)
	...
	= case 'DM'.get taskId sdsEvalStates of
		...
		(Just f) = case f iworld of
			...
			(Ok (res :: AsyncWrite () r^ a^), iworld) = case res of
				Done = (ValueResult (Value val True) evalInfo (ReplaceUI (ui UIEmpty)) st, iworld)
...
where
	...
	= case 'SDS'.modify fun shared ('SDS'.TaskContext taskId) iworld of 
		(Error e, iworld) 						= (ExceptionResult e, iworld)
		(Ok (ModifyResult written), iworld)		= (ValueResult (Value written True) evalInfo  (rep event) (TCStable taskId ts (DeferredJSON written)), iworld)
	...
	eval fun shared event _ tree=:(TCAwait Modify taskId ts _) iworld=:{sdsEvalStates} =  case 'DM'.get taskId sdsEvalStates of
		...
		(Just val) 				= case val iworld of
			...
			(Ok (res :: ModifyResult () r^ w^), iworld) = case res of
				(ModifyResult val) = (ValueResult (Value val True) {TaskEvalInfo|lastEvent=ts,removedTasks=[], refreshSensitive=False} (ReplaceUI (ui UIEmpty)) tree, iworld)
...
where
	...
	eval shared event evalOpts (TCInit taskId=:(TaskId instanceNo _) ts) iworld
		...
		# res = case val of
			Ok (ReadResult val)		= ValueResult (Value val False) {TaskEvalInfo|lastEvent=ts,removedTasks=[],refreshSensitive=True} (rep event) (TCInit taskId ts)
			Error e		= ExceptionResult e
		= (res,iworld)

	eval _ event repAs (TCDestroy _) iworld = (DestroyedResult,iworld)

	eval _ _ _ _ _ = abort "watch does not match!"
