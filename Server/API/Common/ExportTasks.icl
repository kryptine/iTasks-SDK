...
where
	eval taskId iworld=:{taskTime}
		...
		= (ValueResult (Value doc Stable) taskTime (TaskRep (SingleTask,Nothing,[],[]) []) (TCEmpty taskId taskTime), iworld)
...
fileTask taskId filename content f iworld=:{IWorld|taskTime,world}
	...
	= (ValueResult (Value content Stable) taskTime (TaskRep (SingleTask,Nothing,[],[]) []) (TCEmpty taskId taskTime), {IWorld|iworld & world = world})
...
writeDocument taskId filename document iworld
	...
	# (ok,world)			= fclose file world
	| not ok				= (closeException filename,{IWorld|iworld & world = world})	
	= (ValueResult (Value document Stable) taskTime (TaskRep (SingleTask,Nothing,[],[]) []) (TCEmpty taskId taskTime), {IWorld|iworld & world = world})
