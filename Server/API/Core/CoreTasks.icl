...
where
	eval event repOpts (TCInit taskId=:(TaskId instanceNo _) ts) iworld
		...
		# res = case val of
			Ok val		= ValueResult (Value val False) {TaskInfo|lastEvent=ts,expiresIn=Just SHARE_EXPIRY} (finalizeRep repOpts (TaskRep (UIControlSequence (newMap,[],Vertical)) [])) (TCInit taskId ts)
