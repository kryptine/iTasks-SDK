...
where	
	...
	toUITreeContainer` top (TTContainer type tree) iworld = case type of
			...
			# (mbTTContainerType,remainingActions) = case type of
				...
				HiddenTask
					...
					| otherwise // but pass actions upwards in any case
						= (Nothing,actions)
			= ((fmap (\type -> TTContainer type uiTree) mbTTContainerType,remainingActions),iworld)
	...
	mkButtons actions (TTContainer type tree) = TTContainer type (mkButtons` tree)
	where
