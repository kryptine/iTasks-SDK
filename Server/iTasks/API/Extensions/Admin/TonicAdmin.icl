...
showBlueprintInstance rs bpi selDetail enabledSteps compact depth
  ...
  >>~ \outputs -> let outputs` = 'DM'.foldlWithKey (\m (tid, eid) v -> if (tid == bpi.bpi_taskId)
                   ...
                   in updateInformation ()
                        [imageUpdate id (mkInstanceImage rs bpi outputs` enabledSteps selDetail compact) (const id) (\_ _ -> Nothing) (const id)]
                        { ActionState
...
showStaticBlueprint rs bpref task compact depth
  = updateInformation ()
      [imageUpdate id (mkStaticImage rs bpref compact) (const id) (\_ _ -> Nothing) (const id)]
      { ActionState
      | state  = { tis_task    = task
                 , tis_depth   = depth
                 , tis_compact = compact }
