...
isGlobalFunction name
	= contains '.' name 1 // first char can be skipped safely
where
	contains chr str idx
		| idx < size str
			= if (str.[idx] == chr) True (contains chr str (idx+1))
			= False
...
generate_source llmap loader=:(lf,ls) fn world a 
	# (line, llmap, ls, world) = lf ls fn llmap world
	= generate_source_ llmap (lf,ls) fn line world a
where 
	generate_source_ lmap loader fn (Just (LT_REDIRECT name)) world a
		# lmap = del fn lmap // safe to delete because redirect can't link to macro

		// redirect always redirects to the same module, so it is safe to not
		// to try to load the module
		= generate_source_ lmap loader name (get name lmap) world a
	...
	generate_source_ lmap loader fn (Just (LT_MACRO body dt)) world a
		# deps = gendep fn dt
		// macro can't have macro dependency. by design.
		# (lmap, loader, world, a) = foldl (\(lmap, loader, world, a) t = generate_source lmap loader t world a) 
										   (lmap, loader, world, a) deps
		= (put fn (LT_MACRO body DT_NO_DEPENDENCY) lmap, loader, world, a)		
			
	// try to load the module								
	generate_source_ lmap loader fn Nothing world a
		= (lmap, loader, world, a) 
...
sapl_program_name_extension = "sapl"					 
