...
createSessionTaskInstance task event iworld=:{currentDateTime,taskTime}
	...
	# meta					= createMeta instanceNo (Just {SessionInfo|sessionId=sessionId,lastEvent=0}) 0 (Just worker) mmeta pmeta
...
createMeta :: !InstanceNo (Maybe SessionInfo) InstanceNo !(Maybe User) !ManagementMeta !ProgressMeta  -> TIMeta
createMeta instanceNo session parent worker mmeta pmeta
	= {TIMeta|instanceNo=instanceNo,session=session,parent=parent,worker=worker,management=mmeta,progress=pmeta}
		...
		= case mbResultPass1 of
			Ok (_,mbUpdates1)	
				...
				= case mbResultPass2 of
					Ok (result,Just (sessionInfo,updates2))	= (Ok (result,sessionNo,sessionInfo, maybe [] snd mbUpdates1 ++ extraUpdates ++ updates2),iworld)
					Error e					= (Error e, iworld)
					_						= (Error "Unknown error (1) in evalSessionTaskInstance", iworld)
			Error e				= (Error e, iworld)
	| otherwise
		= case mbResultPass1 of
			Ok (result,Just (sessionInfo,updates1))	= (Ok (result,sessionNo,sessionInfo,updates1 ++ extraUpdates),iworld)
			Error e									= (Error e, iworld)
...
refreshTaskInstance instanceNo iworld
	...
	= case mbResult of
		(Ok (_,Just ({SessionInfo|sessionId},updates)))	= addUIUpdates sessionId updates iworld
		_												= iworld
...
evalTaskInstance event instanceNo iworld=:{currentDateTime,currentUser,currentInstance,nextTaskNo,taskTime,localShares,localLists}
	...
	# oldMeta=:{TIMeta|session,parent,worker=Just worker,progress} = fromOk oldMeta
	...
	= case oldResult of
		...
		(ValueResult val _ _ tree)
			//Eval instance
			# repAs						= {TaskRepOpts|useLayout=Nothing,afterLayout=Nothing,modLayout=Nothing,appFinalLayout=isJust session}
