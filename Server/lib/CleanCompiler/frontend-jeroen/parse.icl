...
*/
			_	#	(condefs, pState)	= want_constructor_list exi_vars token pState
					td					= { TypeDef | td & td_rhs = ConsList condefs }
				|	annot == AN_None
	 		  		->	(PD_Type td, pState)
		...
		where
			want_record_type_rhs name is_boxed_record exi_vars pState
				...
				=	(PD_Type { TypeDef | td & td_rhs = SelectorList rec_cons_ident exi_vars is_boxed_record fields }, pState)
	...
	want_type_rhs ColonDefinesToken parseContext td=:{td_attribute} annot pState // type synonym
		# name				= td.td_ident.id_name
		  ...
		  td				= {TypeDef | td & td_rhs = TypeSpec atype}
		|	annot == AN_None
			= (PD_Type td, pState)
	...
	want_type_rhs DefinesColonToken parseContext td=:{td_ident,td_attribute} annot pState
		# name					= td_ident.id_name
		  ...
		  td					= { TypeDef | td & td_rhs = NewTypeCons condef }
		| annot == AN_None
	 		= (PD_Type td, pState)
	...
	want_type_rhs token parseContext td=:{td_attribute} annot pState
		| isIclContext parseContext
			...
			| td_attribute == TA_Anonymous || td_attribute == TA_Unique || td_attribute == TA_None
				...
				# name = td.td_ident.id_name
				= (PD_Type  { TypeDef | td & td_rhs = EmptyRhs cAllBitsClear}, parseError "abstract type" No ("type attribute "+toString td_attribute+" for abstract type "+name+" is not") (tokenBack pState))
