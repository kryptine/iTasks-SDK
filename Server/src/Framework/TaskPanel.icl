...
buildTaskPanels tree menus currentUser = case tree of
	(TTInteractiveTask ti (Left def) acceptedA)
	...
	(TTInteractiveTask ti (Right upd) acceptedA) 
	...
	(TTSequenceTask ti tasks)
		= case [t \\ t <- tasks | not (isFinished t)] of
			...
			[t]	= buildTaskPanels t menus currentUser
			_	= (abort "Multiple simultaneously active tasks in a sequence!")
	(TTParallelTask ti tpi tasks)
		# sttree			= flatten [buildSubtaskPanels t [nr] menus currentUser tpi.TaskParallelInfo.type False \\ t <- tasks & nr <- [1..]] // build subtask containers for all subtasks
...
where
	filterPanel t =
		case t of
			(TTInteractiveTask ti _ _ ) = (fst ti.TaskInfo.worker) == currentUser
			(TTMonitorTask ti _)		= (fst ti.TaskInfo.worker) == currentUser 
			(TTRpcTask ti _)			= (fst ti.TaskInfo.worker) == currentUser
...
buildSubtaskPanels :: !TaskTree !SubtaskNr !(Maybe [Menu]) !UserName !TaskParallelType !Bool -> [SubtaskContainer]
buildSubtaskPanels tree stnr menus manager partype inClosed = case tree of
	(TTInteractiveTask ti (Left def) acceptedA)
		= [{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = STFormPanel {STFormPanel | xtype="itasks.task-form", id = "taskform-" +++ ti.TaskInfo.taskId, taskId = ti.TaskInfo.taskId, items = [def], subtaskId = subtaskNrToString stnr, tbar = (makeMenuBar menus acceptedA ti)}}]	
	(TTInteractiveTask ti (Right upd) acceptedA)
		= [{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = STFormUpdate {STFormUpdate | xtype = "itasks.task-form", id = "taskform-" +++ ti.TaskInfo.taskId, taskId = ti.TaskInfo.taskId, updates = (determineUpdates upd menus acceptedA ti), subtaskId = subtaskNrToString stnr}}]
	(TTMonitorTask ti html)
		= [{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = STMonitorPanel {STMonitorPanel | xtype = "itasks.task-monitor", id = "taskform-" +++ ti.TaskInfo.taskId, taskId = ti.TaskInfo.taskId, html = toString (DivTag [] html), subtaskId = subtaskNrToString stnr}}]
	(TTRpcTask ti rpc)
		= [{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = STMonitorPanel {STMonitorPanel | xtype = "itasks.task-monitor", id = "taskform-" +++ ti.TaskInfo.taskId, taskId = ti.TaskInfo.taskId, html = toString (DivTag [] [Text rpc.RPCExecute.operation.RPCOperation.name, Text ": ", Text rpc.RPCExecute.status]), subtaskId = subtaskNrToString stnr}}]
	(TTFinishedTask ti html)
		= [{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = STMonitorPanel {STMonitorPanel | xtype = "itasks.task-monitor", id = "taskform-" +++ ti.TaskInfo.taskId, taskId = ti.TaskInfo.taskId, html = (foldr (+++) "" [toString h \\ h <- html]), subtaskId = subtaskNrToString stnr}}]
	(TTSequenceTask ti tasks)
		= case [t \\ t <- tasks | not (isFinished t)] of
			[]  = if (allFinished tasks) 
						[{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = TaskDone}] 
						[{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = TaskRedundant}]
			[t] = buildSubtaskPanels t stnr menus manager partype inClosed
			_	= abort "Multiple simultaneously active tasks in a sequence!"
	(TTParallelTask ti tpi tasks)
		# children = zip2 [1..] tasks
		# nmanager = fst ti.TaskInfo.worker
		...
		= case tpi.TaskParallelInfo.type of
			Open 	= flatten [node:[buildSubtaskPanels t [nr:stnr] menus nmanager tpi.TaskParallelInfo.type inClosed \\ (nr,t) <- children]]
			Closed 	= flatten [node:[buildSubtaskPanels t [nr:stnr] menus nmanager tpi.TaskParallelInfo.type True     \\ (nr,t) <- children]]
	(TTMainTask ti mti tasks) 		 
		= case [t \\ t <- tasks | not (isFinished t)] of
			[]  = if (allFinished tasks) 
						...
						[{SubtaskContainer | subtaskNr = stnr, manager = manager, inClosedPar = inClosed, tasktree = tree, taskpanel = TaskRedundant}]
			[t] 
				= buildSubtaskPanels t stnr menus manager partype inClosed
...
where
	...
	buildSubtaskInfo` container = case container.tasktree of
		(TTInteractiveTask ti _ _)
			= {SubtaskInfo | mkSti & taskId = ti.TaskInfo.taskId, subject = ti.TaskInfo.taskLabel, subtaskId = subtaskNrToString container.subtaskNr, delegatedTo = snd ti.TaskInfo.worker}
		(TTMonitorTask ti _)
			= {SubtaskInfo | mkSti & taskId = ti.TaskInfo.taskId, subject = ti.TaskInfo.taskLabel, subtaskId = subtaskNrToString container.subtaskNr, delegatedTo = snd ti.TaskInfo.worker}
		(TTRpcTask ti _)
			= {SubtaskInfo | mkSti & taskId = ti.TaskInfo.taskId, subject = ti.TaskInfo.taskLabel, subtaskId = subtaskNrToString container.subtaskNr, delegatedTo = snd ti.TaskInfo.worker}
		(TTFinishedTask ti _)
			= {SubtaskInfo | mkSti & finished = True, taskId = ti.TaskInfo.taskId, subject = ti.TaskInfo.taskLabel, subtaskId = subtaskNrToString container.subtaskNr, delegatedTo = snd ti.TaskInfo.worker}
		(TTParallelTask ti tpi _)
			= {SubtaskInfo | mkSti & taskId = ti.TaskInfo.taskId, subject = ti.TaskInfo.taskLabel, subtaskId = subtaskNrToString container.subtaskNr, delegatedTo = snd ti.TaskInfo.worker, description = tpi.TaskParallelInfo.description}
