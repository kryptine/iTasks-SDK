...
import Html, Time, iDataForms, iDataTrivial
import TSt

showLabel  		text :== ITag [] [Text text]


locateSubTaskTree :: !TaskId !HtmlTree -> Maybe HtmlTree
locateSubTaskTree taskid (BT _ _) 				= Nothing
locateSubTaskTree taskid (CondAnd label nr [])	= Nothing 
locateSubTaskTree taskid (CondAnd label nr [(desc,tree):trees])
	| taskid == desc.caTaskNrId					= Just tree
	| otherwise	
		= case locateSubTaskTree taskid tree of
			Just tree							= Just tree
			Nothing								= locateSubTaskTree taskid (CondAnd label nr trees)
locateSubTaskTree taskid (tree1 +-+ tree2)
	= case locateSubTaskTree taskid tree1 of
		Just tree								= Just tree
		Nothing									= locateSubTaskTree taskid tree2
locateSubTaskTree taskid (tree1 +|+ tree2)
	= case locateSubTaskTree taskid tree1 of
		Just tree								= Just tree
		Nothing									= locateSubTaskTree taskid tree2
locateSubTaskTree taskid (DivCode _ tree)		= locateSubTaskTree taskid tree	
locateSubTaskTree taskid (TaskTrace _ tree)		= locateSubTaskTree taskid tree		
locateSubTaskTree taskid (desc @@: tree)
	| taskid == desc.taskNrId					= Just (desc @@: tree)
	| otherwise									= locateSubTaskTree taskid tree	
